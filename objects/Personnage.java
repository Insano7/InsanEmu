...
public class Personnage {
	...
	private Map<Integer, Couple<Integer, Integer>> _hechizosSetClase = new TreeMap<Integer, Couple<Integer, Integer>>();
	...
	public Personnage(int _guid, String _name, int _sexe, int _classe,
			...
			int mountXp,int mount,int honor,int deshonor,int alvl,String z, byte title, int wifeGuid, int ScrollFuerza, int ScrollInteligencia, int ScrollAgilidad, int ScrollSuerte, int ScrollVitalidad,
			int ScrollSabiduria)
	{
		this._GUID = _guid;
		this._name = _name;
	...
	public void OnJoinGame()
	{
		...
		if(!CyonEmu.CONFIG_MOTD.equals(""))//Si le motd est notifié
		{
	...
	public void resetearStats() {
		_baseStats.addOneStat(125, -_baseStats.getEffect(125) + _scrollVitalidad);
		_baseStats.addOneStat(124, -_baseStats.getEffect(124) + _scrollSabiduria);
		...
		_baseStats.addOneStat(119, -_baseStats.getEffect(119) + _scrollAgilidad);
		_baseStats.addOneStat(126, -_baseStats.getEffect(126) + _scrollInteligencia);
	}
	...
	public void usePrisme(String packet) {
		...
		if (CarteID == _curCarte.get_id())
			costo = 0;
		if (_kamas < costo) {
			SocketManager.GAME_SEND_MESSAGE(this, "Vous n'avez pas sufisamment de Kamas pour réaliser cette action.",CyonEmu.CONFIG_MOTD_COLOR);
			return;
		}
		_kamas -= costo;
		SocketManager.GAME_SEND_STATS_PACKET(this);
	...
	public void itemagresion(Personnage perso) {
		...
		SocketManager.GAME_SEND_MESSAGE(perso, "Gracias a tu valentía, has recibido " + xp
				+ " puntos de experiencia y " + (5000 * CyonEmu.KAMAS) + " Kamas por haber honorado tu alineación.", "000000");
	...
	public boolean addObjetoConOAKO(Objet objeto, boolean tieneSimilar) {
		for (Entry<Integer, Objet> entry : _items.entrySet()) {
			Objet obj = entry.getValue();
			if (obj.getTemplate().getID() == objeto.getTemplate().getID() && obj.getStats().isSameStats(objeto.getStats())
					&& tieneSimilar && objeto.getTemplate().getType() != 85 && obj.getPosition() == -1) {
	...
	public boolean addObjetoSimilar(Objet objeto, boolean tieneSimilar, int idAntigua) {
		...
		if (tieneSimilar) {
			for (Entry<Integer, Objet> entry : _items.entrySet()) {
				Objet obj = entry.getValue();
				if (obj.getPosition() == -1 && obj.getGuid() != idAntigua && obj.getTemplate().getID() == objModelo.getID()
	...
	public Map<Integer, Couple<Integer, Integer>> getHechizosSetClase() {
	...
	public void addHechizosSetClase(int hechizo, int efecto, int modificacion) {
		if (!_hechizosSetClase.containsKey(hechizo)) {
			_hechizosSetClase.put(hechizo, new Couple<Integer, Integer>(efecto, modificacion));
		}
	}
	public void refrescarSetClase() {
		for (int j = 2; j < 8; j++) {
			...
			int set = obj.getTemplate().getPanopID();
			if (((set >= 81) && (set <= 92)) || ((set >= 201) && (set <= 212))) {
				...
				for (String stat : stats) {
					...
					SocketManager.ENVIAR_SB_HECHIZO_BOOST(this, modificacion);
	...
	public int getModifSetClase(int hechizo, int efecto) {
		...
		if (_hechizosSetClase.containsKey(hechizo)) {
			if (_hechizosSetClase.get(hechizo).first == efecto) {
				modif += _hechizosSetClase.get(hechizo).second;
				return modif;
			}
		}
		return modif;
