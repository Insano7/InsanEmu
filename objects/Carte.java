...
public class Carte {
	...
	public static class Case
	{
		...
		public void startAction(Personnage perso, GameAction GA)
		{
			...
			switch(actionID)
			{
				...
				break;
				case 181:
					SocketManager.ENVIAR_GDF_FORZADO_PERSONAJE(perso, CcellID, 3, 1);
	...
	{
		for(Iterator<Case> iterator = _cases.values().iterator(); iterator.hasNext();)
		{
			Case c = (Case)iterator.next();
			for(Iterator<?> iterator1 = c.getPersos().values().iterator(); iterator1.hasNext();)
			{
				Personnage entry = (Personnage)iterator1.next();
				if(!entry.isOnline())
				{
					SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this, entry.get_GUID());
	...
	public void cleanOfflinePlayersV2()
	{
		for(Iterator<Case> iterator = _cases.values().iterator();iterator.hasNext();)
		{
			...
			if(c != null && c.getPersos() != null)
			{
				...
				{
					Personnage entry = (Personnage)iterator1.next();
					if(entry != null && World.getPersonnage(entry.get_GUID()) == null)
					{
						try
						{
							SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this, entry.get_GUID());
